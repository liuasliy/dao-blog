...
export const dataFormat = (date) => {
    ...
    var preArr = Array.from(Array(10)).flatMap((x, index) => ['0' + index]);
    return year + '-' + (preArr[month] || month) + '-' + (preArr[day] || day) + ' ' +
        (preArr[hours] || hours) + ':' + (preArr[minutes] || minutes) + ':' + (preArr[ses] || ses)
}